...
internal class DeprecatedGeneratorTest : GeneratorTestBase() {
  ...
  companion object {
    ...
    fun testGenerateProvider(): List<Arguments> {
      ...
      return listOf(
        createArgs(sb312, "libs.versions.toml") {},
        createArgs(assertj3242, "name-exclusion-test.toml") { excludeNames = "^assertj-guava$" },
        createArgs(assertj3242, "empty.toml") { excludeGroups = "org\\.assertj" },
        createArgs(assertj3242, "exclusion-negative-test.toml") {
          excludeGroups = "org\\.assertj"
          excludeNames = "xyz"
        },
        createArgs(assertj3242, "both-exclusion-test.toml") {
          excludeGroups = "org\\.assertj"
          excludeNames = "assertj-core"
        },
        createArgs(sb312, "property-overrides.toml") {
          propertyOverrides =
            mapOf(
        ...
        createArgs(sb312, "property-overrides.toml", true) {
          from {
            toml {
              libraryAlias = "springBootDependencies"
              file = Paths.get("src", "test", "resources", "tomls", "source-toml.toml").toFile()
