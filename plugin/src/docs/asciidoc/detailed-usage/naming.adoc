...
functions exposed by the `GeneratorConfig`. The functions can be called multiple times in
...
generator.generate("myLibs") {
  ...
  bundle({ it.versionRef }) { it.versionRef != null && it.alias.contains("dropwizard") } // <3>
}
----
...
<3> If the library's version is a version reference and the alias contains the string `dropwizard`, add it to a bundle
having the name of the version reference
...
generator.generate("myLibs") {
  ...
  it.bundle({ dep -> dep.versionRef }) { dep -> dep.versionRef != null && dep.alias.contains("dropwizard") } // <3>
}
----
<1> If the library's version is a version reference, add it to a bundle where the version reference is the bundle's name
<2> If the library's alias contains the string `junit`, add it to a bundle named `junit`
