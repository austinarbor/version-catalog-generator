...
1. First, upgrade to version 3.4.0 and make sure everything is working as expected
2. A small number of users may be impacted by new alias generation logic where additional "nice" prefixes
...
3. If you weren't using any bundles or using a custom `bundleMapping` function, no further changes should be required.
4. Otherwise
[arabic]
...
----
.. If you had custom a `bundleMapping` definition, replace `bundleMapping = ...` with one or more calls to the new `bundle` functions (see documentation https://austinarbor.github.io/version-catalog-generator/#_customizing_bundle_generation[here])
